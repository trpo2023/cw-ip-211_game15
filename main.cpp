...
const char* YOUWIN = "images/YouWinTexture.jpg"; // Путь к текстуре "Ты Выиграл!"
    ...
    void saveTableToFile() 
    {
        ...
        for (int i = 0; i < records.size(); i++)
        {
            if ((records[i].turns) == "0")
                continue;
            file << records[i].name << "/" << records[i].turns << "/" << records[i].score << "/" << records[i].difficly << std::endl;
        }
        // Закрываем файл
        file.close();
    }
    ...
    while (isMenu && window.isOpen())
    {
        ...
        if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
        {
            if (menuNum == 1) {
                ...
                if (name == "0") continue;
                count = Game(name, diff, BlockSize, (WIGHT / 2) - BlockSize*diff/2,  (HEIGHT / 2) - BlockSize * diff / 2, 1000, window, background, font);
                NewRecord.name = name;
                NewRecord.score = std::to_string(diff * diff * 500 - count * 10);
